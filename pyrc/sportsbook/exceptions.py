...
class InvalidQuery(SportsbookError):
...
class BadStatusCode(SportsbookError):
...
class NoOddsFound(SportsbookError):
...
class NoGameFound(SportsbookError):
...
class NoGameOddsFound(SportsbookError):
    """Could not find odds for the queried team"""
    def __init__(self):
