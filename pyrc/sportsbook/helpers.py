...
def fetch_league_odds_from_api(sport_key,
    ...
    :param str sport_key: The sport to fetch odds for. Valid strings are given
        in the sport_key_map dictionary values.
    :param str bookmaker: Bookmaker to fetch odds from. Default is Unibet.
    :param str odds_api_key: API key for the Odds API — https://the-odds-api.com/
    :return: JSON response from the Odds API.
    :rtype: dict
    """
...
def check_for_valid_query(word_list,
    ...
    :param list word_list: Every word in the user's query.
    :param dict sport_key_map: Dictionary mapping sport names to their
        corresponding sport key.
    :return: None
    :rtype: None
    :raises InvalidQueryError: If the user did not provide a team and valid
        league to query.
    """
    if len(word_list) < 3:
        raise exceptions.InvalidQuery(sport_key_map)
...
def search_for_game_odds(word_list, league_odds):
    ...
    :rtype: dict
    :raises NoOddsFound: If the API returned no odds.
    :raises NoGameFound: If the game was not found in the league odds.
    """
...
def extract_game_info(game_odds):
    ...
    :rtype: tuple
    :raises NoGameOddsFound: If the game odds dict does not have odds from
        the requested bookmaker.
    """
